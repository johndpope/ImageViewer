...
class LightboxTransition: UIPercentDrivenInteractiveTransition {
  ...
  weak var lightboxController: ImageViewerController?
  ...
  @objc func handlePanGesture(_ gesture: UIPanGestureRecognizer) {
    ...
    case .began:
      ...
      if let origin = scrollView?.frame.origin { initialFrame.origin = origin }
    case .changed:
      update(percentage)
      scrollView?.frame.origin.y = initialFrame.origin.y + translation.y
      ...
      if percentage > 0.1 {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.035) {
          UIView.animate(withDuration: 0.35, animations: {
            self.scrollView?.frame.origin = self.initialFrame.origin
          })
        }
      }
    default: break
...
extension LightboxTransition: UIViewControllerAnimatedTransitioning {
  ...
  func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
    ...
    guard let fromView = dismissing ? transitionContext.view(forKey: UITransitionContextViewKey.from) : presentingView,
        let toView = dismissing ? presentingView : transitionContext.view(forKey: UITransitionContextViewKey.to)
      else { return }
